...
const RangeSlider: React.FC<RangeSliderProps> = ({ hasSteps, tooltipVisibility, tooltipPosition, value, onChange, from, to, formatter,setPeopleCount }) => {
    ...
    const updateValue = (closer: HTMLDivElement) => {
        if (railRef.current && trackRef.current && closer && ballSize) {
            ...
            if (closer === minRef.current){ setMin({ value: stringValue.toString(), valueIndex: index }); setPeopleCount(prevState => ({
                ...prevState,
                min: stringValue.toString() // Replace `newMinValue` with the new value for min
              }));}
            if (closer === maxRef.current) {setMax({ value: stringValue.toString(), valueIndex: index }); setPeopleCount(prevState => ({
                ...prevState,
                max: stringValue.toString() // Replace `newMaxValue` with the new value for max
              }));}
        }
